...
/// This is the preferred way of using an consentClient instance.  To complete the integration you must call var <code>general</code> func <code>KVAConsentGeneral/registerIdentity(name:identifier:)</code> followed by func <code>start()</code>.  You may alternatively use a constructor to create your own consentClient.  The shared instance simplifies your implementation as you do not need to store an consentClient instance somewhere in a public location in your own code.
...
/// class MyClass
/// {
///     let consentClient = KVAConsentClient(storageIdentifier: "alternate")
/// }
///
/// \endcode\param storageIdentifier An optional storage identifier.  The storage identifier should be left unset (nil) unless you have a reason to not use the default storage space.  See default constructor KVAConsentClient().
///
...
///
/// \endcode
- (void)registerIdentityWithNameString:(NSString * _Nonnull)nameString valueString:(NSString * _Nonnull)valueString SWIFT_DEPRECATED_MSG("Convenience wrapper function deprecated.  Use Swift func general.registerIdentity(name:identifier:) instead.  If in Objective-C then use [KVAConsentClient.shared.general registerIdentityWithName:identifier:]", "general.registerIdentity(name:identifier:)");
...
///
/// \endcode
- (void)unregisterIdentityWithNameString:(NSString * _Nonnull)nameString SWIFT_DEPRECATED_MSG("Convenience wrapper function deprecated.  Use Swift func general.unregisterIdentity(nameString:) instead.  If in Objective-C then use [KVAConsentClient.shared.general unregisterIdentityWithNameString:]", "general.unregisterIdentity(nameString:)");
/// Start the consent client.
- (void)start;
/// Start the consent client with advanced options.
/// An identity must be registered prior to making this call.  To do this, instead see var <code>general</code> func <code>KVAConsentGeneral/registerIdentity(name:identifier:)</code>.  This method is called by those two methods, and provides public conformance to protocol KVAStartable.  This method can be used to start an instance of a consent client which was created from decoded JSON.
/// important:
...
/// Stop is similar to <code>invalidate()</code> (or the use of KVAConsentProduct/reset(deleteLocalDataBool:) or KVAConsentProduct.shutdown(deleteLocalDataBool:) which wrap <code>invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
...
- (void)stop;
/// Stop the consent client with advanced options.
/// The primary action of this method is to reset the startedTask, which will cause any task which is dependent upon that task to block the initiation of new execution.  The current state of the system is fundamentally maintained;  however, essentially the same things which would not execute prior to having been started will not execute while stopped.
/// Stop is similar to <code>invalidate()</code> (or the use of KVAConsentProduct/reset(deleteLocalDataBool:) or KVAConsentProduct.shutdown(deleteLocalDataBool:) which wrap <code>invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
/// Stop is similar to class KVANetworking var sleepBool in that both will block new execution through a similar mechanism;  however, it is different in that stop essentially covers all aspects of exectution which are at an inverse of start.  In contrast, class KVANetworking var sleepBool exempts certain key features and these can continue to execute while sleep is in effect.
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
...
- (void)invalidate;
/// Invalidate the consent client with advanced options.
...
@property (nonatomic, copy) void (^ _Nullable closure_configuration_didReceive)(KVAConsentConfiguration * _Nonnull);
/// A closure which is called when there is an update to the configuration.
/// This can be used to prompt the user for consent and to enable and/or disable functionality.
@property (nonatomic, copy) KVAConsentConfigurationDidReceiveClosure _Nullable didReceiveConfigurationBlock SWIFT_DEPRECATED_MSG("", "closure_configuration_didReceive");
/// A boolean indicating whether or not the instance has been started.
@property (nonatomic, readonly) BOOL startedBool;
@end


@interface KVAConsentClient (SWIFT_EXTENSION(KochavaConsent)) <JSExport>
@end
...
SWIFT_CLASS_NAMED("KVAConsentDialogNavigationController")
@interface KVAConsentDialogNavigationController : NSObject
/// Create an instance of KVAConsentDialogNavigationController.
/// \param closure_didRequestToOpenURL A closure which is called when an event occurs which requests to open a url.  Set this value to open the specified url.
///
/// \param closure_didComplete A block which is called when the navigation controller’s lifecycle did complete.  Set this value to dismiss the view controller.
///
- (nullable instancetype)initWithClosure_didRequestToOpenURL:(void (^ _Nonnull)(KVAConsentDialogNavigationController * _Nonnull, NSURL * _Nonnull))closure_didRequestToOpenURL closure_didComplete:(void (^ _Nonnull)(KVAConsentDialogNavigationController * _Nonnull))closure_didComplete;
/// Create an instance of KVAConsentDialogNavigationController.
...
///
- (nullable instancetype)initWithDidRequestToOpenURLBlock:(KVAConsentDialogNavigationControllerDidRequestToOpenURLBlock _Nonnull)didRequestToOpenURLBlock didCompleteBlock:(KVAConsentDialogNavigationControllerDidCompleteBlock _Nonnull)didCompleteBlock SWIFT_DEPRECATED_MSG("Modern Objective-C-termed initializer deprecated.  Use Swift init(closure_didRequestToOpenURL:closure_didComplete:) instead.  If in Objective-C then use [[KVAConsentDialogNavigationController alloc] initWithClosure_didRequestToOpenURL:closure_didComplete:]", "initWithClosure_didRequestToOpenURL:closure_didComplete:");
/// Create an instance of KVAConsentDialogNavigationController— using modern Objective-C syntax.
/// note:
/// This method is being phased out and will be removed in a later version of the SDK.  Please instead use func <code>init(didRequestToOpenURLBlock:didCompleteBlock:)</code>.
/// \param didRequestToOpenURLBlock A closure which is called when an event occurs which requests to open a url.  Set this value to open the specified url.
///
/// \param didCompleteBlock A block which is called when the navigation controller’s lifecycle did complete.  Set this value to dismiss the view controller.
///
+ (nullable instancetype)consentDialogNavigationControllerWithDidRequestToOpenURLBlock:(KVAConsentDialogNavigationControllerDidRequestToOpenURLBlock _Nonnull)didRequestToOpenURLBlock didCompleteBlock:(KVAConsentDialogNavigationControllerDidCompleteBlock _Nonnull)didCompleteBlock SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("Modern Objective-C-style factory method deprecated.  Use Swift init(closure_didRequestToOpenURL:closure_didComplete:) instead.  If in Objective-C then use [[KVAConsentDialogNavigationController alloc] initWithClosure_didRequestToOpenURL:closure_didComplete:]", "init(closure_didRequestToOpenURL:closure_didComplete:)");
- (nonnull instancetype)initWithRootViewController:(NSViewController * _Nonnull)rootViewController OBJC_DESIGNATED_INITIALIZER;
...
SWIFT_CLASS_NAMED("KVAConsentDialogViewController")
@interface KVAConsentDialogViewController : NSViewController
/// A navigation controller.
/// This declaration exists for API compatibility with macOS and watchOS.  In those cases this class is not a sub-class of UIViewController and does not have this property, but there is a need at this time for the property to consistently exist.  Note that this does not provide functionality at this time, but is a placeholder for future functionality.
@property (nonatomic, strong) id _Nullable navigationController;
...
/// <h2>Example</h2>
/// \code
/// KVAConsentClient.shared.general.unregisterIdentity(name: "_SOME_NAME_")
///
/// \endcode
